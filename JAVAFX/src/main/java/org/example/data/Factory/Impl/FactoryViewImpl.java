...
public class FactoryViewImpl<T> implements FactoryView<T> {
    ...
    public FactoryViewImpl(Class<T> clazz, ClientService clientService, ArticleService articleService,DetailService detailService, 
        ...
        if (Client.class.isAssignableFrom(clazz)) {
            ...
            this.view = (View<T>) new DeptView(scanner, (ClientServiceImpl) clientService, 
                                               (DeptServiceImpl) deptService, (ArticleServiceImpl) articleService,(DetailServiceImpl) detailService);
        } else {
