...
public class Main {
    ...
    private static void initializeUsers(UserServiceImpl userServiceImpl) {
        if (userServiceImpl.findAll().isEmpty()) {
            System.out.println("Initialisation des utilisateurs par défaut...");

            User admin = new User(1, "Baila WANE", "bbw_login", "b", Role.ADMIN, TypeEtat.ACTIVER);
            User boutiquier = new User(2, "BOBO", "bobo_login", "b", Role.BOUTIQUIER, TypeEtat.ACTIVER);
            User client = new User(3, "Client 1", "client_login", "b", Role.CLIENT, TypeEtat.ACTIVER);
            // Ajouter les utilisateurs au service
            userServiceImpl.create(admin);
            userServiceImpl.create(boutiquier);
            userServiceImpl.create(client);

            System.out.println("Utilisateurs par défaut initialisés avec succès.");
        } else {
    ...
    public static void main(String[] args) {
        ...
        FactoryRepository<Paiement> factoryRepositoryPaiement = new FactoryRepositoryImpl<>(Paiement.class);
        ...
        var paiementRepository = factoryRepositoryPaiement.createRepository();
        ...
        FactoryService<Paiement> factoryServicePaiement = new FactoryServiceImpl<>(Paiement.class, paiementRepository);
        ...
        PaiementServiceImpl paiementServiceImpl = (PaiementServiceImpl) factoryServicePaiement.createService();
        ...
        // initializeUsers(userServiceImpl); // a commenter si vous n'utiliser pas la
        // version List dans le config.yaml
                ...
                case BOUTIQUIER -> {
                    ...
                    do {
                        ...
                        handleBoutiquierMenu(choice, clientServiceImpl, userServiceImpl, articleServiceImpl, clientView,
                                userView, deptServiceImpl, deptView, paiementServiceImpl);
                    } while (choice != 12); 
                }
                case CLIENT -> {
                    // Menu CLIENT
        ...
        while (attempts < MAX_ATTEMPTS) {
            ...
            if (user != null) {
                ...
                if (attempts < MAX_ATTEMPTS) {
                    System.out.println("Il vous reste " + (MAX_ATTEMPTS - attempts) + " tentative(s).");
                }
            }
        }
        System.out.println("Vous avez fait plusieurs échecs de connexion. Veuillez réessayer plus tard.");
        return null;
    }
    ...
    private static void handleBoutiquierMenu(int choice, ClientServiceImpl clientServiceImpl,
        ...
        switch (choice) {
            ...
            case 8 -> {
                System.out.println("1- Afficher les dettes en cours");
                System.out.println("2- Afficher les dettes Annuler");
                int filterChoice = scanner.nextInt();
                if (filterChoice == 1) {
                    List<Dept> listDebts = deptServiceImpl.findByEtat(TypeDette.ENCOURS);
                    if (listDebts != null && !listDebts.isEmpty()) {
                        listDebts.forEach(System.out::println);
                    } else {
                        System.out.println("=====================.");
                        System.out.println("Aucune dette en cours.");
                        System.out.println("=====================.");
                    }
                } else if (filterChoice == 2) {
                    List<Dept> listDebts = deptServiceImpl.findByEtat(TypeDette.ANNULER);
                    if (listDebts != null && !listDebts.isEmpty()) {
                        listDebts.forEach(System.out::println);
                    } else {
                        System.out.println("=====================.");
                        System.out.println("Aucune dette Annuler.");
                        System.out.println("=====================.");
                    }
                }
            }
            case 9 -> {
                System.out.println("1- Accepter une demande de dette");
                System.out.println("2- Annuler une demande de dette");
                int filterChoice = scanner.nextInt();
                if (filterChoice == 1) {
                    deptView.AccepterDept();
                } else if (filterChoice == 2) {
                    deptView.AnnulerDept();
                }
            }
            case 10 -> {
                List<Dept> listDepts = deptServiceImpl.findAll();
                listDepts.forEach(System.out::println);
            }
            case 11 -> {
                System.out.println("Paiement d'une dette");
                ...
                Dept dept = deptServiceImpl.findById(idDette);
                if (dept == null) {
                ...
                paiement.setClient(dept.getClient());
                paiement.setMontant(montantPaiement);
                paiement.setDept(dept);

                double montantRestant = dept.getMontantRestant() - montantPaiement;
                dept.setMontantVerser(dept.getMontantVerser() + montantPaiement);
                dept.setMontantRestant(Math.max(montantRestant, 0));
                dept.updateEtat();
            ...
            case 12 -> System.out.println("Déconnexion...");
            default -> System.out.println("Choix invalide.");
        }
    }
        ...
        switch (choice) {
            ...
            case 3 -> {
                ...
                if (connectedUser != null && connectedUser.getRole() == Role.CLIENT) {
                    ...
                    System.out.println("2- Afficher ses dettes ENCOURS OU ANNULER");
                    int filterChoice = scanner.nextInt();
                    if (filterChoice == 1) {
